...
namespace KidzGame.Bootstrap
{
    ...
    /// <see cref="ConfirmPopup"/> prefab via <see cref="PopupRef.Direct"/> (no Addressables
    ...
    {
        [Tooltip("A prefab with a ConfirmPopup component (see com.kidzdev.unity.popup/Runtime/Popups/ConfirmPopup.cs).")]
        ...
        public override async UniTask<bool> ConfirmAsync(long totalBytes)
        {
            ...
            return await _popupManager.ShowAsync<bool>(PopupRef.Direct(confirmPopupPrefab), message);
        }
