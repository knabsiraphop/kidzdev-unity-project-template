...
- **Everything must live under `Assets/` (or `Packages/`).** A folder at the
  ...
  not compile, no matter how correct the C# inside it is. `_Project/` lives at
  `Assets/_Project/`, not at the repo root, for exactly this reason.
...
2. **`GameBootstrap.cs`** (`Assets/_Project/Runtime/Bootstrap/GameBootstrap.cs`)
...
3. **`AddressablesUsageValidator`** (from `com.kidzdev.unity.project-conventions`)
   ...
   the toolkit's own assembly or `_Project/Runtime/Bootstrap/`. The exemption
   is a **literal path-string match** on `/_Project/Runtime/Bootstrap/` —
